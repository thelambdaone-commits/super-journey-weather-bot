...
for f in files:
    if "venv" in str(f) or ".gemini" in str(f) or "tests/audit" in str(f): continue
...
grade(
    ...
    len(bad) == 0,
    bad if bad else "No obvious lookahead patterns"
)
...
for f in files:
    if "venv" in str(f) or ".gemini" in str(f) or "tests/" in str(f) or "run_full_audit" in str(f): continue
...
grade(
    "broad_exceptions",
    len(bad) == 0,
    bad if bad else "No broad except Exception blocks"
)
...
for f in files:
    if "venv" in str(f) or ".gemini" in str(f): continue
    if "src/trading" not in str(f) and "src/strategy" not in str(f) and "src/backtest" not in str(f): continue
    txt = f.read_text(errors="ignore").lower()
...
grade(
    ...
    len(bad) < 5, 
    bad if bad else "Fees/slippage references found in core trading logic"
)
...
grade(
    "paper_mode_boot",
    rc == 0,
    err if rc else "paper mode launches"
)
