...
class TelegramNotifier:
    ...
    def notify_trade_open(self, city: str, date: str, bucket: str, price: float, 
        ...
        header = "⚡ *EXÉCUTION LIVE CLOB*" if is_live else "🚀 *PARI SIMULÉ OUVERT*"
    ...
    def notify_hourly_report(self, summary: dict, city_signals: list[dict]) -> bool:
        ...
        mode = str(summary.get("mode") or "LIVE").upper()
        if mode == "PAPER":
            equity = summary.get('paper_equity', 0.0)
            balance = summary.get('paper_balance', 0.0)
            exposure_line = (
                f"→ Equity: `{balance:,.2f}$` (Balance: `{balance:,.2f}$` + Open: `{equity - balance:,.2f}$`)\n"
                f"→ Max DD: `{summary['drawdown']:.2f}%` | Expo Paper: `{summary.get('paper_exposure', 0.0):.2f}$`\n"
            )
        else:
            exposure_line = (
                f"→ Max DD: `{summary['drawdown']:.2f}%` | Expo Live: `{summary['exposure']:.2f}$` "
                f"| Paper: `{summary.get('paper_exposure', 0.0):.2f}$`\n"
            )
        # Format optional metrics
        pf_str = f"{summary['pf']:.2f}" if summary.get('pf') is not None else "N/A"
        sharpe_str = f"{summary['sharpe']:.2f}" if summary.get('sharpe') is not None else "N/A"
        ...
        msg = (
            ...
            f"→ Profit Factor: `{pf_str}` | Sharpe: `{sharpe_str}`\n"
