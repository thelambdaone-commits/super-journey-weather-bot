...
def kelly_fraction_binary(probability: float, price: float) -> float:
    ...
    Kelly = (b*p - q) / b = (p - price) / (price * (1 - price))
    """
...
def fractional_kelly_binary(probability: float, price: float, fraction: float = 0.25) -> float:
    """Fractional Kelly (default 25%)."""
    return fraction * kelly_fraction_binary(probability, price)
...
def final_position_size(
    ...
    fk = fractional_kelly_binary(probability, entry_price, fraction)
    raw_size = max(0.0, fk * bankroll)

    # Check daily drawdown
    if daily_pnl < -bankroll * max_dd:
        return SizingResult(
            kelly_fraction=kelly_fraction_binary(probability, entry_price),
            fractional_kelly=fk,
            raw_size=raw_size,
            capped_size=0.0,
            final_size=0.0,
